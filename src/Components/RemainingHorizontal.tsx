...
const RemainingHorizontal = ({ ...props }) => {
  ...
  //     .then(res => {
  //       console.log("cxvcvxcvcxvcxvcxv",res.data?.kwh_unit_remaining)
  ...
  const remainigUsuageData = () => {
    ...
    axios
      ...
      .then(res => {
        ...
        const total = parseFloat(res.data?.total_kwhunit || '0');

   

        setTotalAllowed(total);

        // ✅ Use a threshold (example: anything less than 0.05 is treated as 0)
        if (remaining > 0.05) {
          remaingData = remaining.toFixed(2);
          dispatch(setOverUsage(false));
          dispatch(setOverusageCount(0));
        } else if (overusage > 0) {
    ...
    <>
      <View
        style={{
          ...
          marginVertical: DIMENSIONS.SCREEN_HEIGHT * 0.0,
          flexDirection: 'row',
          shadowColor: '#000000',
